...
export default function AddUserForm({ users, onCreated }) {
  ...
  const handleCreate = async () => {
    ...
    if (!isValidPassword(form.password)) {
        newErrors.password = 'Mot de passe ≥ 8 caractères avec majuscule, minuscule et chiffre.';
    }
    ...
    setCreating(true); setErrors({}); setError(''); setMsg('');
      ...
      <div style={s.formGrid}>
        ...
        <div style={s.formGrp}>
          ...
          <input style={s.formInp} value={form.username}
            onChange={update('username')} placeholder="ex: etudiant_test" />
        </div>

        <div style={s.formGrp}>
          <label style={s.formLbl}>Email *</label>
          <input style={s.formInp} value={form.email}
            onChange={update('email')} placeholder="test@est.ma" />
            {errors.email && (
        ...
        <div style={{ ...s.formGrp, position: 'relative' }}>
          ...
          />
          {form.password && (
            <button type="button" onClick={() => setShow(v => !v)} style={st.eyeBtn}>
